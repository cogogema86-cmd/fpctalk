        ...
        {STEPS.map((step, i) => (
          ...
          >
            <button
              ...
              className="relative bg-zinc-50 dark:bg-zinc-900 aspect-[9/16] flex items-center justify-center hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors p-2"
            >
              <img
                src={step.image}
                alt={`Step ${i + 1}`}
                ...
                loading="lazy"
              />
              <span className="absolute top-1.5 left-1.5 inline-flex items-center justify-center w-7 h-7 rounded-full bg-emerald-600 text-white text-sm font-bold shadow">
                {i + 1}
              </span>
            </button>
            <div className="p-3 text-base leading-snug text-zinc-700 dark:text-zinc-300">
              {t(step.key)}
            </div>
          </li>
        ))}
