...
## ✅ 2026-08-31 (4차) — **원인 확정**: iOS Safari엔 scroll anchoring이 없다 (`6fad41a`)
> 3차 롤백 후에도 재보고: "제일 하단에 안 내려가고 하단에서 조금 위에서 멈춰있습니다." → 스크린샷으로 **롤백본이 맞음**을 확인. 즉 **제 수정 이전부터 있던 문제**였다.

- **재현에 성공** (이번 건의 전환점): DB 없이 되는 순수 클라이언트 레이아웃 문제라, 실제 마크업 + 지연 응답하는 실제 JPEG(`sharp`로 생성, 서버에서 350ms 지연 + `Cache-Control: no-store`)로 하네스를 만들어 재현했다.
- **원인**: 진입 직후 콘텐츠 높이가 계속 자란다 — 첨부 이미지가 `loading="lazy"`인데 **높이 예약이 없어 로드 전 높이가 0에 가깝다**. **Chrome은 scroll anchoring으로 자란 만큼 자동 보정하지만 iOS Safari는 이 기능을 지원하지 않아** 그대로 바닥에서 밀려난다.
  - ⚠️ **데스크톱에서는 절대 재현되지 않는다.** 이것이 1~3차에서 세 번 헛짚은 직접적 이유. 하네스에서 `overflow-anchor: none`을 줘야 비로소 증상이 나온다.
  - 기존 보정 코드(`load` 이벤트 + 5초 재고정)는 **화면을 한 번 탭(`touchstart`)하기만 해도 죽도록** 돼 있어 폰에서는 사실상 동작하지 않았다.
- **측정** (scroll anchoring 끈 상태 = iOS 조건):

  | | 최종 위치 | 바닥 | 차이 | scrollHeight 변화 |
  |---|---|---|---|---|
  | 수정 전 | 2684 | 3109 | **425px 부족** | 1680→3709 (**16단계**) |
  | 수정 후 | 2800 | 2800 | **0** | **2단계** |

  사용자가 직접 끌어올린 경우(500) 그 위치 유지 — 강제로 끌어내리지 않음도 확인.
- **수정** (`_chat-room.tsx` 1개 파일):
  1. 첨부(사진·동영상)에 **정사각 고정 틀** — 로드 전에도 높이 확정. 첨부 metadata에 원본 크기가 없어 CSS로만 예약 가능하고, 덕분에 **기존 메시지에도 소급 적용**된다.
  2. 무엇이 자라는지 추측하지 말고 **`ResizeObserver`로 관측**해 재고정. **탭이 아니라 실제로 끌었을 때(`touchmove`/`wheel`/사용자 스크롤)만 중단.** 4초 상한.
  3. `initialScrollDoneRef`를 "끝났는가" 한 뜻으로 통일 (2차 내용). 첫 줄에서 곧바로 `true`로 만들면 선언 순서상 뒤 effect가 마운트에서 가드를 통과해 smooth 스크롤을 발사한다.
  4. 읽은 기록 없는 첫 입장은 맨 아래 (카톡 동작).
  5. 안읽음 구분선을 **인덱스 대신 id로** — 계산은 `initialMessages`, 렌더는 `visibleMessages`(진행중 ORDER 제외) 기준이라 인덱스 비교는 ORDER가 있을 때 엉뚱한 메시지에 붙었다.
- **검증**: 위 하네스 측정 3종 + tsc 0 + build 성공 + eslint 24건(기준선 동일).
- ⚠️ **실기기 확인 필요**: `.env`를 아직 못 받아 실제 앱으로는 미확인. 사용자 요청으로 선배포. **PWA는 완전 종료 후 재실행해야 새 번들이 온다.**
- **교훈 (이번 건의 핵심)**: 모바일에서만 나는 렌더링 버그를 **데스크톱 브라우저로 검증하고 배포-확인 루프를 돌린 것**이 세 번의 헛수고를 만들었다. 다음에 "폰에서만 이상하다"는 보고를 받으면 **먼저 폰 조건을 재현할 방법부터 만들 것** — 이 건에서는 `overflow-anchor: none` 한 줄이 그 열쇠였다.

---

## ⏪ 2026-08-31 (3차) — 위 두 건 **코드 롤백**. 원인 재조사 중
> 2차 배포 후 재보고: "오히려 제일 하단으로 안 내려가고 위에 올라가있습니다." → 사용자 요청으로 `_chat-room.tsx`를 `cde0563` 시점으로 **완전 복구**(diff 0). WORK_LOG의 위 두 항목은 조사 기록으로 남겨두되 **코드는 배포돼 있지 않다**.

- **새로 확보한 결정적 단서**: 사용자 확인 — 그 화면에 **"여기서부터 안 읽음" 구분선이 보이지 않음**. 즉 `firstUnreadIdx === -1`이라 `applyScroll`의 **"맨 아래로" 경로가 실행**됐는데도 위에 머물렀다는 뜻. → **rAF 시점의 `scrollHeight`가 최종값보다 작았다**가 확정.
- **1·2차 수정의 실제 효과와 한계**:
  - 1차(첨부 높이 예약): 이미지 시프트는 실측 0으로 제거됨. 유효한 수정. 다만 **나중에 커지는 요소가 이미지만이 아니었다** — 한글 웹폰트(`@fontsource/noto-sans-kr`, 서브셋이 커서 모바일에서 늦게 스왑) 리플로우 등이 남는다.
  - 2차(가드 버그): `initialScrollDoneRef` 겸용 문제는 실재하는 버그였고 수정도 맞다(StrictMode 재현으로 확인). **그런데 이 버그가 우연히 보정 역할을 하고 있었다** — 마운트 시 발사되던 smooth 스크롤이 뒤늦게 맨 아래로 끌어내려 주고 있었다. 5초 재고정 루프도 같은 역할. 둘을 다 없애니 **아무도 교정하지 않는 상태**가 되어 "위에 멈춤"이 드러났다.
- **다음 수정 방향(미적용)**: 무엇이 커지는지 추측하지 말고 **관측**할 것. 스크롤 콘텐츠에 `ResizeObserver`를 걸어 크기가 변하는 동안(제한 시간 내) 목표 위치로 계속 재고정하고, 사용자가 직접 스크롤하면 즉시 중단. 이미지 높이는 이미 예약돼 있으니 남은 변동폭이 작아 재고정이 눈에 띄지 않는다. 노출은 `document.fonts.ready`(타임아웃 포함) 이후로.
- ⚠️ **근본 제약 — 이게 3번 헛다리를 짚은 진짜 이유**: 로컬에 `.env`가 없어 **앱을 띄워 재현할 수 없다**. 매번 배포해서 사용자가 확인하는 루프로 디버깅하는 중. 다음에 이 영역을 건드리면 **먼저 로컬 실행 환경부터 확보할 것**(`vercel env pull` 또는 개발용 DB).
- **덤으로 발견한 별개 버그(미수정)**: `firstUnreadIdx`는 `initialMessages` 기준으로 계산하는데(`:208`) 렌더는 `visibleMessages` 기준으로 비교한다(`:1053` `showDivider = i === firstUnreadIdx`). 진행 중 ORDER 메시지가 있으면 그 뒤 인덱스가 1씩 밀려 **구분선이 엉뚱한 메시지에 붙는다**.

---

## ✅ 2026-08-31 (2차) — 진입 시 smooth 자동 스크롤이 발사되던 가드 버그 (`e1dd651`)
> 1차 배포 후 재보고: "핸드폰으로 채팅방에서보니 아직도 스크롤돼서 내려가는데". 원인이 **두 개**였고 1차는 그중 하나(레이아웃)만 잡았다.

- **원인 — `initialScrollDoneRef`를 두 가지 뜻으로 겸용**: 초기 스크롤 effect는 첫 줄에서 "시작했다"는 뜻으로 곧바로 `true`를 넣는데, 뒤의 "새 메시지 자동 스크롤"(`:259`)·"읽음 처리"(`:271`) effect는 "끝났다"는 뜻으로 읽는다. **React는 effect를 선언 순서대로 같은 마운트에서 실행**하므로 뒤 effect가 마운트에서 가드를 통과 → `scrollTo({ behavior: "smooth" })` 발사 → 최상단에서 맨 아래까지 애니메이션.
  - 목록 노출(`scrollReady`)은 rAF 2프레임 뒤인데 애니메이션은 그 **전에** 시작되므로, 사용자는 진행 중인 애니메이션을 그대로 본다. 데스크톱에선 rAF 즉시 스크롤이 먼저 이겨서 안 보이고 폰에서만 보였음 — 그래서 1차 때 놓쳤다.
- **수정** (`_chat-room.tsx`, 3줄):
  - `initialScrollDoneRef`를 "끝났는가" 한 가지 뜻으로 통일 — 위치를 다 잡은 rAF #2에서만 `true`.
  - 중복 실행 방지 가드(`if (ref.current) return`) 제거. deps `[]`라 마운트당 한 번뿐이고, StrictMode 이중 실행 때는 오히려 재실행이 맞다. **그대로 뒀으면 두 번째 실행이 즉시 return하고 rAF는 cleanup으로 취소돼 목록이 `opacity-0`인 채 안 보이는 상태**가 됐을 것(1차 수정으로 새로 생길 뻔한 위험).
- **검증**: React 18 + StrictMode로 수정 전/후를 나란히 마운트해 이벤트 기록 — 수정 전 `["SMOOTH SCROLL FIRED", "revealed"]`(노출 전에 애니메이션 시작), 수정 후 `["revealed"]`. 양쪽 최종 위치 6160/6160, opacity 1. tsc 0, build 성공, eslint 24건으로 기준선 동일.
- **배제한 것**: 서비스워커 캐시(`public/sw.js`는 pass-through, 캐싱 안 함), 다른 폴링 effect(모두 `prev.map`이라 `messages.length` 불변 → 재발화 없음).
- ⚠️ **실기기 확인 필요**: 여전히 `.env`가 없어 로컬 재현 불가. **PWA는 앱을 완전히 종료 후 재실행해야 새 번들을 받는다** — 백그라운드에 떠 있던 화면은 구버전 JS 그대로다. 1차 보고 때 이 가능성도 배제 못 했음.
- **알려진 남은 이슈(미수정, 요청 시 처리)**: 히스토리를 위로 올려 읽는 중에 새 메시지가 오면 `:259`가 맨 아래로 끌어내린다. 카톡은 안 그런다 — "하단 근처에 있을 때만 따라간다"로 바꾸려면 별건으로.
- **교훈**: 증상이 하나여도 원인이 하나라는 보장이 없다. 1차 때 "레이아웃 시프트 0" 측정에 만족하고 **진입 경로 전체를 훑지 않은 게 실수**. effect 가드는 ref 이름이 뜻하는 시점과 실제 대입 시점이 같은지 항상 확인할 것.

---

## ✅ 2026-08-31 — 채팅방 진입 시 스크롤이 최상단부터 훑고 내려가던 문제 (`a7ac228`)
> 요청: "채팅방 들어가면 상단 글 내용부터 쭉 내려옵니다. 카톡처럼 마지막 보던 위치나, 전체를 봤다면 제일 하단에 있어야 하는데." (화면녹화 첨부)
> ⚠️ **재발 건**: 이전 `채팅방 재입장 시 읽던 위치 복원 안 되는 문제 수정` 커밋과 같은 증상. 그때는 위치 계산(`lastReadAt`·구분선)을 고쳤고, 이번 원인은 **레이아웃**이라 별개다.

- **읽은 위치 기억 기능은 이미 있었음**: `ChatMember.lastReadAt` → `getChatInfo`의 `myLastReadAt` → `firstUnreadIdx`(안읽음 구분선) → 진입 시 구분선으로 스크롤. 최종 도착 위치는 맞았다.
- **진짜 원인 — 첨부 이미지에 예약 높이가 없음** (`_chat-room.tsx:2157` 등): `<img>`에 width/height도 aspect 박스도 없어서 진입 시점 `scrollHeight`가 실제의 1/10 수준. "맨 아래로" 보내도 사실상 히스토리 최상단이고, 이후 이미지가 하나씩 디코딩되며 위쪽 높이가 불어나 뷰포트가 아래로 걸어 내려갔다. `:236-252`의 5초 재고정 루프가 그 과정을 사용자에게 그대로 노출. 사진이 많을수록 길어짐.
  - 화면녹화 프레임으로 확정: t=6s 전체 대화가 한 화면에 압축(빈 말풍선+`267KB`만) → t=7s 8/28 중간을 훑고 지나감 → t=8s 맨 아래 안착.
- **부수 문제**: `lastReadAt`이 `null`인 첫 입장자는 `firstUnreadIdx = 0` → 진짜 최상단에서 시작했음.
- **수정** (`_chat-room.tsx` 1개 파일, +29/-33):
  - 첨부 3곳에 정사각 고정 틀 — 사진 1장 `w-[240px] sm:w-[320px] aspect-square`+`object-contain`, 그리드 타일 `aspect-square`+`object-cover`, 동영상 동일. **첨부 metadata에 원본 크기가 없어 CSS로만 예약 가능** → 그 대신 기존 메시지에도 소급 적용된다.
  - 5초 재고정 루프·`touchstart`/`wheel` 중단 리스너 제거(높이가 안 변하므로 불필요). rAF 2회로 위치 확정 후 `scrollReady`로 `opacity-0 → 100` 노출 — 웹폰트 등 잔여 시프트까지 안 보임.
  - 첫 입장은 `firstUnreadIdx = -1` → 맨 아래(카톡 동작). 구분선은 읽은 기록이 있는 재입장에서만.
- **검증**: tsc 0, build 성공, eslint 24건이 **HEAD 기준선과 동일**(stash 후 비교 — 신규 0건). 레이아웃 시프트 실측(Chromium, 수정 전/후 마크업 나란히 렌더): 수정 전 26px→320px(**사진 1장당 +294px**), 수정 후 320px→320px(**0**), 그리드·동영상도 0.
- ⚠️ **실기기 확인 필요**: 로컬에 `.env`가 없어 앱을 띄우지 못함 → 3가지 진입 경우(첫 입장 / 다 읽고 재입장 / 안 읽은 메시지 있음)를 실제로 확인하지 못했다. 배포 후 확인할 것.
- **알려진 변화**: 세로로 긴 사진이 정사각 틀 안에 들어가 이전보다 작게 보인다. 어색하면 `aspect-square`를 `aspect-[4/5]` 등으로 조정.
- **다음에 비슷한 증상이 나오면**: 위치 계산(`lastReadAt`)부터 의심하지 말고 **새로 추가된 가변 높이 요소(첨부·임베드·번역 토글 등)에 예약 높이가 있는지** 먼저 볼 것.

---

## ✅ 2026-07-27 — 세션 만료 시 채팅 업로드·전송 실패를 명확히 안내 (`771be47`)
> 문의: "특정 계정에서 채팅방 이미지 업로드가 안 됨" → 재로그인으로 해결됨. 권한 문제가 아니라 세션 만료였음.

- **조사 결과 — 계정별 업로드 제한은 없음**: `app/api/chat/[chatId]/upload/route.ts`의 게이트는 ①로그인 여부 ②채팅방 접근 권한(멤버 or `role.defaultLevel >= levelRequired`) 둘뿐. 역할(PRINCIPAL/VICE)·isAdmin 분기 없음. 저장소도 계정별이 아닌 `STORAGE_PROVIDER` 환경변수로 전역 결정(`lib/storage.ts:29`).
- **진짜 원인**: 세션 만료. `middleware.ts:54-58`이 미인증 요청을 **전부** `/login`으로 리다이렉트하는데 matcher에 `/api/chat/...`도 포함 → 업로드 라우트의 401은 브라우저에서 **도달하지 않음**. 클라이언트는 `err.error` 없는 응답을 받아 `"업로드 실패"` 기본 문구만 표시 → 사용자가 기능 고장으로 오인.
  - 세션 정책: `lib/supabase/server.ts:7-58` — `fpctalk-rememberMe=1` 쿠키 있으면 30일 maxAge, 없으면 브라우저 닫으면 삭제되는 session cookie. (로그인 폼 기본값은 rememberMe ON)
- **수정** (`_chat-room.tsx` 1개 파일, +42/-1):
  - `isAuthExpired(res)` — **401 그리고 `/login` 리다이렉트(`res.redirected`) 둘 다 감지**. 401만 보면 미들웨어 때문에 실제로 안 잡힘.
  - 업로드/전송 실패가 세션 만료면 파일별 오류 나열 대신 `"로그인이 만료되었습니다. 다시 로그인해주세요."` 하나만 표시. 여러 장 동시 업로드 시 문구 반복 방지.
  - 서버 액션 경로도 동일 처리 (`sendMessageAction`의 `"로그인이 필요합니다."` 감지).
  - 안내 2초 후 `window.location.href = "/login"` — `router.push`가 아닌 전체 로드로 만료 세션에 물린 클라이언트 상태(Realtime 채널 등) 초기화. `loginRedirectRef`로 중복 예약 방지.
- **검증**: tsc 0, eslint 5err+1warn으로 **변경 전 baseline과 동일**(stash 후 비교 — 신규 0건, 라인만 31줄 밀림), build 성공.
- **미해결/알려진 한계**: 자동 이동이라 **입력 중이던 텍스트는 유실됨**(어차피 전송 불가 상태). 로그인 후 `loginAction`이 항상 `/chat` 목록으로 보내므로 **보던 채팅방으로 복귀하지 않음** — `next` 파라미터 지원은 범위 밖이라 미작업.
- ⚠️ **실기기 확인 필요**: 세션 만료 상황 재현이 까다로움. 다음에 같은 증상 발생 시 문구가 바뀌었는지 확인.
...
- **캠페인 삭제 admin 전용**: `lib/documents.ts deleteCampaign` 권한을 isAdmin → **PRINCIPAL 역할(code) 전용**으로 변경 (`isUserPrincipal` 헬퍼 추가). 부원장(VICE)도 isAdmin이라 기존엔 본인 캠페인 삭제 가능했음 → 이제 불가.
  - **기존 '본인 캠페인만' 소유자 체크는 제거**: 부원장이 만든 캠페인을 아무도 못 지우는 상태 방지 — admin은 작성자 무관 삭제 가능. (단 목록 `getDocumentsByUploader`는 여전히 본인 것만 표시 — UI상 admin이 남의 캠페인을 볼 일은 아직 없음.)
  - **UI**: `documents/page.tsx`에 `isPrincipal`(role.code === "PRINCIPAL") 추가, 캠페인 삭제 버튼을 isPrincipal일 때만 렌더. 서버 액션은 deleteCampaign 내부 체크로 이중 방어.
  - 양식(템플릿) 삭제·사인요청 취소 권한은 무변경(기존 isAdmin+본인 것).
- **10개 단위 페이지네이션은 이미 구현·배포 상태**(`e9d61ce` 섹션 접기 + 페이지네이션, `_collapsible-section.tsx` pageSize 기본 10): 양식 보관함·진행 캠페인·사인 대기·사인 완료 섹션 모두 10개 초과 시 ← n/N → 컨트롤. 추가 작업 없음.
- **검증**: tsc 0, 변경 3개 파일 eslint 0(전체 14err은 기존 사전이슈 — react-hooks 규칙, 미수정), build 성공. ⚠️ 실기기(admin/부원장 계정별 버튼 노출) 확인 필요.

---

## ✅ 2026-07-09 — 문자 발신번호를 학원 유선번호(031-991-2500)로 변경 (코드 변경 없음 · 재배포 · 실수신 확인완료)
> 사용자가 솔라피 콘솔에 학원 번호 031-991-2500을 발신번호로 등록 완료 → 프로그램 발신번호를 기존 010-9456-8432에서 교체.

- **코드 수정 없음**: `lib/sms.ts`의 01X 형식 검증은 수신자 번호에만 적용, 발신번호는 숫자만 정규화해 전달 → 유선번호 그대로 사용 가능.
- **변경 내역**: 로컬 `.env` `SOLAPI_SENDER=0319912500` + Vercel production 환경변수 교체(`vercel env rm/add`) 후 production 재배포(`vercel redeploy`, www.fpctalk.com alias 확인).
- **검증**: 사용자가 실제 사인 요청 문자 발신번호 031-991-2500으로 수신 확인.
- **메모**: 발신번호를 다시 바꾸려면 코드 아닌 환경변수(`SOLAPI_SENDER`, 로컬+Vercel)만 교체하고 재배포하면 됨. 유선번호는 솔라피 콘솔 승인 상태여야 발송됨.

---

## ✅ 2026-07-08 — 채팅 읽던위치·사진스와이프 + 사인요청 문자발송(솔라피)·즐겨찾기·양식편집 (`d576714`~`b909019` 배포·실기기 확인완료)
> 한 세션에 8건 배포. 전부 main 자동배포 후 **사용자 갤럭시+아이폰 실기기 확인**. 스와이프·읽던위치·문자수신 정상 확인됨.

### (1) 채팅방 진입 시 읽던 위치부터 열기 — 1차 수정 (`d576714`)
- **증상**: "계속 중간 부분부터 보여짐."
- **원인**: `lib/chat.ts getChatMessages`가 `orderBy asc + take 200` = **가장 오래된 200개**만 로드. 200개 초과 방에서 최근 메시지·안읽음 구분선이 로드 범위 밖 → 항상 옛날 지점 표시.
- **수정**: `desc + take 200 + reverse()` — 최신 200개를 시간순으로. 기존 구분선/맨아래 스크롤 로직 그대로 살아남.

### (2) 이미지 뷰어 좌우 스와이프 — 채팅방 전체 사진 (카톡 방식) (`d576714`)
- **요청**: "핸드폰에서 사진 하나 보고 옆으로 넘겨서 보기." 범위는 채팅방 전체 사진 선택.
- **구현**: 뷰어 상태를 말풍선별 → `ChatRoom` 레벨 단일 뷰어로 리프트. `galleryItems`(로드된 메시지의 비만료 이미지 평면 목록) + `onOpenImage(messageId, attIdx)`. `_image-viewer.tsx`: items+initialIndex, 터치 스와이프(가로 50px↑ & 가로>세로), 데스크톱 ‹›버튼+←→키, 상단 `n / N` 카운터, 인접사진 미리로드, 공유 File은 URL과 함께 저장(사진 넘긴 직후 이전 파일 오공유 방지).
- **한계**: 로드된 최근 200개 메시지 범위 내 탐색.

### (3) "[사진 N장]" 자동 문구 말풍선에서 숨김 (`d576714`)
- 첨부만 보내면 서버가 검색·푸시용 content(`[사진 N장]`/`[첨부 N개]`/파일명)를 자동 생성 — 화면에 그대로 노출됨. `showContentText`가 자동 생성 규칙과 일치하면 숨기도록 변경(DB·검색·푸시는 유지).

### (4) 갤럭시(좁은 화면) 채팅방 헤더 깨짐 (`d576714`)
- **증상**: 제목이 세로 한글자씩. 관리자 버튼들(자동응답/초기화/방삭제)이 `shrink-0`이라 제목 영역(flex-1 min-w-0)이 0폭으로 압축.
- **수정**: 헤더 `flex-wrap`(공간 부족 시 버튼 다음 줄) + 제목 `min-w-[10rem]` + 부제목 `truncate`.

### (5) 외부 사인 요청 → 사인 링크 문자 자동 발송 (솔라피) (`ab87437`, 한/영 병기 `1b279cc`)
- **배경**: 외부 사인자 이메일/전화 입력칸은 기록용일 뿐 아무것도 발송 안 됐음. 문자천국은 DB에이전트 방식(고정IP+설치형)이라 Vercel과 연동 불가 → 솔라피 가입(테스트 발송으로 키 검증 후 구현).
- **구현**: `lib/sms.ts` — 솔라피 v4 send, HMAC-SHA256 인증, 번호 정규화/검증(01X), env 미설정 시 조용히 스킵. `template-actions.ts` 외부 요청 생성 후 전화번호 있는 사인자에게 링크 문자(`Promise.allSettled`, 실패해도 요청 유지). 문구 한/영 병기(제목=양식명은 관리자가 한/영 작성).
- **환경변수**: `SOLAPI_API_KEY/API_SECRET/SENDER` — 로컬 `.env` + **Vercel production 등록 완료**(vercel env add). SETUP_GUIDE에 안내 추가. 발신 01094568432. 링크 포함이라 대부분 LMS(45원/건).

### (6) 사인 요청 화면 직원 목록 접기 — 기본 접힘 (`6645b6b`)
- 직원 많아지면 지저분 → 헤더가 토글(▶), 기본 접힘, 선택 인원수 상시 표시. **접힘=display:none** (체크박스가 DOM에 남아야 선택·폼제출 유지). 전체선택은 접힌 채로도 동작. 외부 사인자 영역 무변경.

### (7) 외부 사인자 즐겨찾기 — 정식 주소록 (`2d840a8` 이력기반 → `1b279cc` 정식 대체)
- 1차(발송 이력 기반 칩)는 "문자 보내야 생김" 피드백으로 **명시적 즐겨찾기로 교체**.
- **DB**: `ExternalContact` 모델(name / email·phone은 `@default("")` — null이면 unique 무력화 / `@@unique([name,email,phone])`). `prisma db push` 반영 완료.
- **UI**: 외부 사인자 우측 `⭐ 즐겨찾기` 버튼 → 체크박스 패널(여러 명 체크 → "체크한 N명 추가" 일괄 등록, 삭제 버튼). 등록은 요청 목록 각 사람 옆 `☆ 저장`. 액션 `addExternalContactAction`(upsert)/`deleteExternalContactAction` (관리자 전용).

### (8) 사인 페이지 문서 전체보기 — pdf.js 폭맞춤 (`1b279cc` CSS 시도 → `b96a369` 근본 해결)
- **증상**: 문자 링크로 들어가면 문서가 크게 확대·잘려 보임. CSS(A4 비율 iframe)로는 해결 안 됨 — **iOS 사파리는 iframe PDF를 원본 크기로 잘라 표시(축소 불가)**.
- **수정**: `_pdf-viewer.tsx` — pdf.js **legacy 빌드**(최신 빌드는 `Map.getOrInsertComputed` 등 신규 JS API로 구형 엔진에서 죽음)로 각 페이지를 캔버스에 화면폭 맞춤 렌더(dpr≤2), 전체 페이지 세로 나열, 확대는 핀치줌. 이미지 첨부는 `w-full h-auto`(높이 제한 제거).
- **CORS**: `api/sign-files`가 R2 서명 URL로 리다이렉트 → CORS 헤더 없어 pdf.js fetch 차단. `?stream=1` 추가(리다이렉트 대신 same-origin 스트리밍). **Playwright 실브라우저 검증**(A4 비율 캔버스 렌더, 콘솔에러 0) 후 배포. 의존성 `pdfjs-dist` 추가.

### (9) 양식 편집 기능 (`13a4bd2`)
- 양식 목록에 사인요청/삭제만 있고 편집 없음 → `✏️ 편집` 버튼 + `/documents/templates/[id]/edit`.
- 이름·설명 수정(기존값 프리필), 파일은 **새로 첨부한 경우에만 교체**(현재 파일명 표시, 비우면 유지). 기존 파일 미삭제(기존 캠페인이 같은 경로 참조), 교체 시 양식의 기존 storageType 유지(ko/en 저장소 분산 방지). `updateTemplate`/`updateTemplateAction`. 이미지 축소 헬퍼 `_image-compress.ts`로 공용화.

### (10) 읽던 위치 2차 수정 — 재입장 시 위에서 열리던 잔여 버그 (`b909019`)
- **증상**: 끝까지 읽고 나갔다 재입장(또는 앱 재시작)하면 위쪽에서 열림 (이미지 포함 방에서 특히).
- **원인 2가지**: ① `markAsRead`가 입장 시 1회만 호출 — 대화 중 도착한 메시지는 화면으로 봐도 안읽음으로 남아 재입장 시 그 위치에 구분선. ② 맨아래 스크롤 직후 위쪽 이미지 로드로 내용이 늘어나 위치가 위로 밀림.
- **수정**: ① 방에 있는 동안 새 메시지 도착 1.5초 후 markAsRead(디바운스). ② 진입 후 5초간 이미지 `load`(capture)마다 목표 위치 재적용, 사용자가 터치/휠 시작하면 즉시 중단.

- **검증**: 매 건 tsc 0 / eslint 신규 0(기존 5err+1warn 유지) / build 성공. (8)은 Playwright 실렌더 검증. 문자 실수신, 스와이프·읽던위치·헤더는 실기기(갤럭시+아이폰) 확인.
- **메모**: 카톡 인앱브라우저가 옛 페이지를 물고 있어 배포 직후엔 브라우저 완전 종료 후 재접속 필요. 이메일 자동 발송은 아직 없음(입력값은 기록용) — 필요 시 Resend 등으로 추가 가능.

---

## ✅ 2026-06-22 — 채팅 사진 다중 업로드 + 입력칸 고정 + 이미지 뷰어 (`0806ff4`, `4d1c343` 배포·실기기 확인완료)
> 한 세션에 3건 + 후속 1건. 전부 main 배포 후 **사용자 아이폰에서 정상 동작 확인**.

### (1) 채팅 첨부 여러 장 한 번에 업로드 — 갤러리 (`0806ff4`)
- **요청**: "사진 한 장씩만 올라가는데 여러 장 한 번에." → 사용자가 '한 메시지 묶음(갤러리)' 선택.
- **구현**: 메시지 metadata에 `attachments[]`(배열) 새 표준. 옛 단일 `attachment`는 읽기 시 `[attachment]`로 정규화(하위호환, 회귀 없음). 최대 10개/메시지(`MAX_ATTACHMENTS_PER_MESSAGE`).
  - `lib/chat.ts sendMessage`: `attachments` 옵션, 타입=전부 이미지면 IMAGE 아니면 FILE, content 폴백 `[사진 N장]`/`[첨부 N개]`.
  - `chat/actions.ts`: formData `attachments`(JSON 배열) 파싱·검증(+단일 하위호환).
  - `_chat-room.tsx`: input `multiple`, `handleFilesSelect`(`Promise.allSettled` 병렬 업로드), `pendingAttachments[]`+칩 N개 개별삭제, 갤러리 렌더. 드래그앤드롭도 다중.
  - `api/chat/file/[messageId]/route.ts`: `?i=N` 인덱스 서빙(기본 0).
  - `api/cron/cleanup-attachments`: 배열 전체 순회·삭제, 첨부별 expiresAt 부분만료 지원.

### (2) 모바일 입력칸이 화면 밖으로 밀려 사라지던 버그 (`0806ff4`)
- **증상**: iOS에서 입력칸(+하단 탭)이 안 보이고 연필 눌러야 나타남.
- **근본원인**: 앱셸 높이 오류. `(main)/layout.tsx` 최상위 `min-h-screen`(불확정) + `chat/[id]`·`assistant` 페이지 `h-[calc(100vh-58px)]`(정적 vh=iOS 동적툴바 미반영, 상단헤더+하단탭 중 한쪽만 차감) → 세로합이 viewport 초과, in-flow `MobileNav`까지 밀림.
- **수정**: 표준 앱셸로. `layout.tsx` `min-h-screen`→`h-dvh` + body row/`<main>`에 `min-h-0`. 두 페이지 `h-[calc(100vh-58px)]`→`h-full min-h-0`(매직넘버 제거). 기존 `overflow-hidden/auto` 의도대로 작동→헤더·탭 고정, 본문만 내부 스크롤. (전 페이지 스크롤이 문서→`<main>` 내부로. InstallBanner는 `fixed`라 무관.)

### (3) 이미지 탭 → 앱 내 뷰어 (X 닫기 + 공유/저장) (`0806ff4`)
- **증상**: 이미지 '다운로드'→iOS 네이티브 다운로드 화면으로 현재 화면 통째 이동 → 스탠드얼론 PWA라 갇힘, 미리보기 들어가면 사이트 재접속해야 탈출.
- **원인**: 다운로드 링크 same-frame attachment 이동이 PWA 유일 뷰를 덮어씀. Apple 네이티브 화면엔 X 못 그림.
- **수정**: 이미지 탭 시 iOS로 안 보내고 **앱 내 라이트박스**(`_image-viewer.tsx`) — ✕/배경/Esc 닫기(body 스크롤 잠금), 하단 '공유/저장'=**Web Share API**(`navigator.share({files})`)로 공유시트(카톡·사진저장) 직행. 제스처 내 동기호출 위해 **마운트 시 blob 미리 fetch**(같은출처 `?download=1` 스트림=CORS 무관), 미지원 시 새 탭 폴백. 이미지 `<a target=_blank>`→`<button onClick=openViewer>`, iOS행 '📥다운로드' 텍스트 제거. i18n `chat.imgClose/imgShare/imgSharePreparing` KO/EN. (동영상·일반파일은 `target=_blank`만.)

### (4) 후속: 갤러리 이미지 잘림 제거 (`4d1c343`)
- **피드백**: 여러 장이 "캡처한 느낌"(정사각 크롭)으로 보임. 카톡처럼 사진 전체가 보여야 함.
- **수정**: 그리드 썸네일 `aspect-square object-cover`→`w-full h-auto max-h-[300px] object-contain`, 그리드에 `items-start`. 같은 비율 사진은 같은 높이로 나란히, 잘림 없음. (탭→단일 뷰어는 (3)에서 이미 동작.)

- **검증**: tsc 0, build 성공, eslint 신규 0(기존 사전이슈 미수정). **사용자 아이폰 실기기에서 4건 모두 정상 확인.**
...
- **i18n**: `admin.attendance.summary.*`(title/empty/units/cols) KO/EN, 종류명은 기존 `leave.type.${ENUM}` 재사용.
- **검증**: tsc 0, build 성공, 배포 후 페이지 정상(미인증 리다이렉트). ⚠️ Playwright 끊겨 화면 검증은 사용자 폰 확인 필요.
